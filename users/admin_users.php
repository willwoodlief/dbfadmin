...
$email = null;
$form_valid=FALSE;
$permOpsQ = $db->query("SELECT * FROM app_permissions");
  ...
  $validation->check($_POST,array(
    ...
    'matches' => 'password',
      ...
      try {
        ...
        $db->insert('app_users',$fields);
        $theNewId=$db->lastId();
        // bold($theNewId);
        $addNewPermission = array('user_id' => $theNewId, 'permission_id' => $perm);
        $db->insert('app_user_permission_matches',$addNewPermission);
        $db->insert('app_profiles',['user_id'=>$theNewId, 'bio'=>'This is the bio']);
