...
function fetchUserjsonPIE()
	...
	FROM app_permission_page_matches LEFT JOIN app_permissions ON permission_id = app_permissions.id  GROUP BY permission_id",[sum1,name]);
...
function fetchUserjsonLG()
	...
	// Example query
	$db = DB::getInstance();
	$stmt = $db->query("SELECT COUNT(*) AS sum1,sign_up_stamp	FROM app_users GROUP BY DAY(FROM_UNIXTIME(sign_up_stamp))ORDER BY sign_up_stamp DESC");
	$stmt->execute();
...
// complex query gets audit
	function fetchAllLatest($userid,$start,$end,$eventcode)
		...
		$stmt = $db->query("SELECT
									...
									audit_itemid,
									audit_timestamp
			FROM audit LEFT JOIN app_users ON audit_userid = id
									WHERE
									audit_userid != ?
									AND audit_timestamp >= ?
									AND audit_timestamp <= ?
...
//Retrieve information for admin audit
	function fetchAllAudit()
		...
		$stmt = $db->query("SELECT
				...
				audit_itemid,
				audit_timestamp
			FROM audit LEFT JOIN app_users ON audit_userid = id ORDER BY audit_id DESC");
			$results = $stmt->results();
			$count = $stmt->count();
			return $results;
			return $count;
	...
	function fetchUserAudit($userid)
		...
		$stmt = $db->query("SELECT
			...
			FROM audit LEFT JOIN app_users ON audit_userid = id WHERE audit_userid = ? ORDER BY audit_id DESC",[$userid]) ;
			$results = $stmt->results();
			$num_returns = $stmt->count();
			return $results;
			return $num_returns;
